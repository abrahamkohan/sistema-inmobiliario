...
export function AdminOnboardingPage() {
  ...
  if (created) {
    const metadata = JSON.stringify({ consultant_id: created.uuid }, null, 2)
    ...
    return (
      <div className="max-w-xl mx-auto p-6 flex flex-col gap-6">
        ...
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 flex flex-col gap-3">
          <p className="text-sm font-semibold text-amber-800">Próximo paso — invitar al usuario</p>
          <ol className="text-sm text-amber-900 flex flex-col gap-2 list-decimal list-inside">
            <li>Andá a <strong>Supabase Dashboard → Authentication → Users</strong></li>
            <li>Hacé click en <strong>Invite user</strong></li>
            <li>Ingresá el email del cliente</li>
            <li>En <strong>Additional metadata</strong> pegá esto:</li>
          </ol>
          <div className="flex items-start gap-2 mt-1">
            <pre className="flex-1 text-xs font-mono bg-white border rounded-lg px-3 py-2 text-gray-700 overflow-x-auto">
              {metadata}
            </pre>
            <button
              onClick={() => copyToClipboard(metadata)}
              className="flex-shrink-0 flex items-center justify-center w-9 h-9 rounded-lg border bg-white hover:bg-gray-50 transition-colors mt-0.5"
          ...
          <p className="text-xs text-amber-700 mt-1">
            El sistema asigna automáticamente al usuario a este tenant cuando acepta la invitación.
          </p>
        </div>
    ...
    <div className="max-w-xl mx-auto p-6 flex flex-col gap-6">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          {subdomain && (
            <p className="text-xs text-muted-foreground">{subdomain}.sistema.com</p>
          )}
        </div>
